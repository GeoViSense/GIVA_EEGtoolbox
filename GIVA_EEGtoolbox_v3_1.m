function varargout = GIVA_EEGtoolbox_v3_1(varargin)
% giva_eegtoolbox_v3_1 MATLAB code for GIVA_EEGtoolbox_v3_1.fig
%      giva_eegtoolbox_v3_1, by itself, creates a new giva_eegtoolbox_v3_1 or raises the existing
%      singleton*.
%
%      H = giva_eegtoolbox_v3_1 returns the handle to a new giva_eegtoolbox_v3_1 or the handle to
%      the existing singleton*.
%
%      giva_eegtoolbox_v3_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in giva_eegtoolbox_v3_1.M with the given input arguments.
%
%      giva_eegtoolbox_v3_1('Property','Value',...) creates a new giva_eegtoolbox_v3_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GIVA_EEGtoolbox_v3_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GIVA_EEGtoolbox_v3_1_OpeningFcn via varargin.
%
...
%
% Edit the above text to modify the response to help GIVA_EEGtoolbox_v3_1
%
% Last Modified by GUIDE v2.5 02-Jun-2018 23:10:58
%
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @GIVA_EEGtoolbox_v3_1_OpeningFcn, ...
    'gui_OutputFcn',  @GIVA_EEGtoolbox_v3_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before GIVA_EEGtoolbox_v3_1 is made visible.
function GIVA_EEGtoolbox_v3_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GIVA_EEGtoolbox_v3_1 (see VARARGIN)

% Choose default command line output for GIVA_EEGtoolbox_v3_1
...
% UIWAIT makes GIVA_EEGtoolbox_v3_1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);



% --- Outputs from this function are returned to the command line.
function varargout = GIVA_EEGtoolbox_v3_1_OutputFcn(hObject, eventdata, handles)
...
switch measure_type
    ...
    case 3 % Engagement index
        mean_engage = zeros(size(eeg_spectrum,1),1);
        for count = 1:size(eeg_spectrum,1)
            file{count} = eeg_spectrum{count}.file;
            ...
            mean_engage(count,1) = mean(mean(scores{count}.mean_engage_across_channels,1));
        end
    ...
    case 4 % Arousal
        mean_arousal = zeros(size(eeg_spectrum,1),1);
        for count = 1:size(eeg_spectrum,1)
            file{count} = eeg_spectrum{count}.file;
            mean_arousal(count,1) = Arousal(eeg_spectrum{count}.n_epoch, eeg_spectrum{count}.stim_spectrum, eeg_spectrum{count}.baseline_spectrum);
        end
